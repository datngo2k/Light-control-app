...
class ActionRespository {
  ...
  Future<ActionState> getAction(String roomId, List<Bulb> bulbs, List<Sensor> sensors, DateTime datetime) async {
    try {
      ActionGetAllSuccess state = ActionGetAllSuccess(bulbs:[], sensors: []);
      for(Bulb bulb in bulbs){
        await getBulbAction(roomId, bulb);
  ...
  Future<void> getBulbAction(String roomId, Bulb bulb) async {
    await _actionRef
        .child(roomId)
        .child("bulb")
        .child(bulb.id)
      ...
      if (dataSnapshot.value != null) {
        try {
          ...
          bulb.actions = [];
          for (var key in keys) {
            ActionDevice action = ActionDevice.fromSnapshot(key, bulb.id, value[key]);
            if (action != null) {
  ...
  Future<void> getSensorAction(String roomId, Sensor sensor, DateTime datetime) async {
    ...
    await _actionRef
      ...
      if (dataSnapshot.value != null) {
        try {
          ...
          var value = dataSnapshot.value;
          for (var key in keys) {
            ActionDevice action = ActionDevice.fromSnapshot(key, "Sensor", value[key]);
